...
def Buckling_check(Nx,Ny,Ns,Dmatrix,a = 1,b = 1,m = 1):
     ...
     print(f'R_buckling: {R_buckling}')
     print(f'Shear ratio: R_s: {R_s}')
     print(f'Compressive ration: R_c: {R_c}')
...
def is_MaxStress_FPF(Laminate):
    ...
    if not isFPF:
        for idx, ply in enumerate(Laminate.plys):
            ...
            if ply.loading_ratio > loading_ratio:
                loading_ratio = ply.loading_ratio
...
# print("D-matrix")
# print(f'D: {black_aluminum_laminate.D}')
...
t_composite_arr = np.array([t_middle, t_diagonal, t_diagonal, t_top])

# initialise loadvector for ABD
#n_x_arr, n_s_arr = calc_loadvector(V = +V_y, M = M_x, t_arr = t_composite_arr) # tensile (top half of fuselage)
n_x_arr, n_s_arr = calc_loadvector(V = +V_y, M = M_x, t_arr = t_composite_arr) # compresive (bottom half of fuselage)
n_x_arr *= -1 # changes tension to compression
# n_x_arr = np.zeros(4)
# n_s_arr = np.zeros(4)
n_y = 0
m_x = 0
...
isFPF_top, loading_ratio_top = is_MaxStress_FPF(top_laminate)
isFPF_diagonal,  loading_ratio_diagonal = is_MaxStress_FPF(diagonal_laminate)
isFPF_middle,  loading_ratio_middle = is_MaxStress_FPF(middle_laminate)

print('FPF Check')
print(f'Top Laminate FPF?: {isFPF_top}, loading_ratio: {loading_ratio_top}')
print(f'Diagonal Laminate FPF?: {isFPF_diagonal}, loading_ratio: {loading_ratio_diagonal}')
print(f'Middle Laminate FPF?: {isFPF_middle}, loading_ratio: {loading_ratio_middle}')
...
print(f'Thickness [mm]: {t_composite_arr*10**3} [mm]')
