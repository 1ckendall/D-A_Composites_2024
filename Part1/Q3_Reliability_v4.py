        ...
        for j in range(iterations): 
            ...
            while firstplyfailureoccurence == False and N < (N_max):
                ...
                for idx, ply in enumerate(plylist_ijk):
                    ...
                    #         if ply_damage.failuremode == "FFT" or ply_damage.failuremode == "FFC" or ply_damage.failuremode == "IFF A" or ply_damage.failuremode == "IFF B" or ply_damage.failuremode == "IFF C":
                    #             failuretracking += 1
                    #             firstplyfailureoccurence = True
                    #             Pf_arr[i,j] = 1/N
...
Pf_mean = np.mean(Pf_arr[n_vars_active])
Pf_std = np.std(Pf_arr[n_vars_active])
...
lower_error =  np.zeros(count_n_vars_active)
upper_error =  np.zeros(count_n_vars_active)
for i in range(count_n_vars_active):
    if Pf_mean>Pf_vars_mean[n_vars_active][i]:
        lower_error[i] = Pf_mean-Pf_vars_mean[n_vars_active][i]
    elif Pf_mean<=Pf_vars_mean[n_vars_active][i]:
        upper_error[i] = Pf_vars_mean[n_vars_active][i]-Pf_mean
# asymmetric_error = np.array(list(zip(lower_error[n_vars_active], upper_error[n_vars_active]))).T
...
plt.clf()
plt.title(figname)
plt.errorbar(np.arange(count_n_vars_active), Pf_mean*np.ones(count_n_vars_active), asymmetric_error, color = 'blue', fmt='o', ecolor='red', capsize=6)
# plt.errorbar(np.arange(count_n_vars_active), Pf_vars_mean[n_vars_active], asymmetric_error, color = 'blue', fmt='o', ecolor='red', capsize=6)
plt.axhline(Pf_mean, color='blue', linestyle='--', label=f'Probability of Failure: {Pf_mean}')
plt.xticks(np.arange(count_n_vars_active), UD_names[n_vars_active], fontsize = 16)
