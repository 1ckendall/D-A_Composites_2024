...
def create_circle(num_points,R):
    ...
    for i in range(1, num_points):
        arc_lengths[i] = 2*np.pi*R/(num_points-1) #np.sqrt((x[i] - x[i-1])**2 + (y[i] - y[i-1])**2)

    interp_points = 100  # Number of points after interpolation
...
# calculate stress due to moment on the discritised length
X[0],Y[0]
x[0],y[0]
x[-1],y[-1]
...
for i in range(num_points):
    sig_s1 = ((M_x*y[i-1])/Ixx_t)*T[0]
    sig_s2 = ((M_x*y[i-num_points+1])/Ixx_t)*T[0]
    sig_boom = ((M_x*y[i])/Ixx_t)*T[0]

    # Boom Area = Stringer Area + skin 1 contribution + skin 2 contribution
    Sai = A_stiffner[0]
    B1i = (T[0]*arc_length[0]/6)*(2+(sig_s1/sig_boom))
    B2i = (T[0]*arc_length[0]/6)*(2+(sig_s2/sig_boom))
    B_area = Sai+B1i+B2i
    Ixx_boom = (y[i]**2)*B_area
    B_bending.append(Ixx_boom)

Ixx_full=np.sum(B_bending)
sig_z=M_x*y/Ixx_boom
